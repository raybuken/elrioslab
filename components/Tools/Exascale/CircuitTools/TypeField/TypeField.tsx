import React, { useId } from 'react'
import { CircuitTypes } from '../../data/exascale'
import styles from './type-field.module.css'

function TypeField({onChangeType}: {onChangeType: (e: React.ChangeEvent<HTMLInputElement>) => void}) {
    const id = useId()
    const types = [CircuitTypes.I, CircuitTypes.L, CircuitTypes.T, CircuitTypes.REPLICATED]
    
    const EXA_TYPES = {
        [CircuitTypes.I]: 'Type I',
        [CircuitTypes.L]: 'Type L',
        [CircuitTypes.T]: 'Type T',
        [CircuitTypes.REPLICATED]: 'Replicated',
    }

    return (
        <div className={styles.typeField}>
            {types.map((type, index) => 
                <div className={styles.typeContainer} key={index} >
                    <label htmlFor={`${id}-${type}`}>{EXA_TYPES[type]}</label>
                    <input 
                        className={styles.type} 
                        type="radio" 
                        name={`${id}-type`}
                        id={`${id}-${type}`} 
                        defaultValue={type} 
                        defaultChecked={index === 0} 
                        onChange={onChangeType}
                    />
                </div>
            )}
        </div>
    )
}

export default TypeField